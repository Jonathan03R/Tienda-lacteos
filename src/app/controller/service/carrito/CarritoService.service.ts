...
export class CarritoServiceService {
  ...
  agregarAlCarrito(producto: Productos) {
    ...
    const existe = this.carrito.some(item => item.ProductoCodigo === producto.ProductoCodigo);
    if (!existe) {
  ...
  eliminarDelCarrito(codigoProducto: number) {
    const index = this.carrito.findIndex(item => item.ProductoCodigo === codigoProducto);
    if (index !== -1) {
